/**
 * ClinicOS: ClinicoAI Symptom Triage & Clinical Assistant
...
class TriageEngine {
  ...
  openTriage() {
        ...
        <div class="incoming-call-modal" style="width:540px; max-width:92vw; text-align:left; padding:1.5rem; display:flex; flex-direction:column; height:580px;">
          <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--border-subtle); padding-bottom:0.75rem; margin-bottom:0.75rem;">
            <div style="display:flex; align-items:center; gap:0.6rem;">
              ...
              <div>
                <strong style="color:var(--text-navy); font-size:1.05rem;">ClinicoAI Triage</strong>
    ...
    this.step = 0;
    this.messages = [
      { role: 'bot', text: 'Hello! I am your ClinicoAI assistant. How are you feeling today? You can select a common symptom below or describe what you are experiencing.' }
    ];
    this.renderChat();
  ...
  renderChat() {
    ...
    body.innerHTML = this.messages.map(m => `
      <div style="display:flex; gap:0.5rem; align-self:${m.role === 'user' ? 'flex-end' : 'flex-start'}; max-width:85%;">
        ...
        <div style="background:${m.role === 'user' ? 'var(--primary)' : 'var(--bg-subtle)'}; color:${m.role === 'user' ? '#ffffff' : 'var(--text-main)'}; padding:0.65rem 0.95rem; border-radius:var(--radius-md); font-size:0.84rem; line-height:1.45; border:${m.role === 'user' ? 'none' : '1px solid var(--border-card)'};">
          ${m.text}
        </div>
      </div>
    `).join('');
  ...
  processSymptomLogic(text) {
    ...
    if (lower.includes('chest') || lower.includes('heart') || lower.includes('palpitation') || lower.includes('flutter')) {
      ...
      this.messages.push({
        role: 'bot',
        text: `<strong>Clinical Triage Assessment: Dermatology OPD</strong><br><br>Possible contact dermatitis or allergic reaction. Specialist doctors are available on video call in under 10 minutes.<br><br>
